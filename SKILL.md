---
name: paper-format
description: 将纯文本/带简单结构标记的论文草稿自动排版为标准格式 Word 文档。触发词：论文格式化、格式化论文、排版论文、生成论文 Word。
---
...
将用户输入的论文草稿（纯文本，无 Markdown 标记），一键转换为符合参考论文格式的 Word 文档。

**输入**：论文草稿文本（中文标题 + 英文标题 + 关键词 + 摘要 + 正文章节）
**输出**：已排版的 `.docx` 文件，保存至 `{OUTPUT_DIR}`（默认 `~/Desktop/论文/output/`）
**模板**：`{TEMPLATE_PATH}`（默认 `{skill_dir}/论文/论文1_明杆闸阀丝杠开度识别.docx`）
...
```
[中文大标题]              黑体 15pt，居中，段前2行、段后0.5行，单倍行距
[空一行]
[摘要：] + 摘要正文        摘要/内容宋体 10.5pt，段前0行、段后0行，无缩进
[关键词：] + 内容         关键词黑体、内容宋体 10.5pt，段前0.5行
[空一行]
[英文大标题]              Times New Roman 14pt，居中，段前0行、段后6磅
[Abstract:] + 内容       Times New Roman 10.5pt，段前0行、段后0行，无缩进
[Keywords:] + 内容        Times New Roman 10.5pt，段前0行、段后0行，无缩进
[引言]                    Heading 2，黑体 12pt
[正文段落]                Normal，宋体 10.5pt，两端对齐，首行缩进2字符
[一级章节 1 xxx]          Heading 2，宋体 10.5pt
[二级章节 1.1 xxx]        Title，宋体 10.5pt
[参考文献 标题]           Heading 2，宋体 10.5pt
[参考文献条目]            List Paragraph，宋体 10.5pt，无首行缩进
```

**页面设置**：A4，上下边距 2.15 cm，左右边距 1.95 cm
**页眉**：无（已清空）
**分栏**：引言开始至文档末尾采用等宽双栏（栏间距 0.85 cm）；中文大标题、摘要（中英文）、关键词保持单栏
...
INPUT = os.environ.get(
    'PAPER_FORMAT_INPUT',
    os.path.join(_tempfile.gettempdir(), 'paper_input.txt')
)
...
def add_para(doc, style_name, text):
    p = doc.add_paragraph()
    p.style = doc.styles[style_name]
    if text:
        run = p.add_run(text)
        apply_rPr(run, get_style_rPr(doc, style_name))
    return p
...
def format_paper(input_path, output_filename):
    ...
    new_order.append(('_blank', ''))                   # 空一行
    if abs_cn_block: new_order.append(abs_cn_block)    # 摘要
    if kw_cn_block: new_order.append(kw_cn_block)     # 关键词
    new_order.append(('_blank', ''))                   # 空一行
    if en_title_text: new_order.append(('title_en', en_title_text))
    if abs_en_block: new_order.append(abs_en_block)    # Abstract
    if kw_en_block: new_order.append(kw_en_block)     # Keywords
    ...
    for btype, text in new_order:
        if btype == '_blank':
            # 空一行：段前360twips≈1行高，加零宽字符防被清理删除
            p = doc.add_paragraph()
        ...
        if btype == 'title_cn':
            # 黑体15pt，居中，段前2行(600twips)，段后0.5行(210twips)，单倍行距(360twips)
            p = doc.add_paragraph()
            ...
            # 摘要：黑体标题10.5pt + 宋体内容10.5pt，段前0行、段后0行、无缩进
            p = doc.add_paragraph()
            p.style = doc.styles['Normal']
            ...
            # 关键词：黑体标题10.5pt + 宋体内容10.5pt，段前0.5行(210twips)，单倍行距
            p = doc.add_paragraph()
            p.style = doc.styles['Normal']
            ...
            # 黑体加粗 14pt，居中，段前0行、段后6磅(120twips)
            p = doc.add_paragraph()
            p.style = doc.styles['Normal']
            ...
            # Abstract 正文：Times New Roman 10.5pt，段前0行、段后0行、无缩进，单倍行距
            p = doc.add_paragraph()
            p.style = doc.styles['Normal']
            set_para_spacing_para(p, before_twips=0, after_twips=0, line_twips=360, first_indent_twips=0)
            ...
            # Keywords：Times New Roman 10.5pt，段前0行、段后0行、无缩进，单倍行距
            p = doc.add_paragraph()
            p.style = doc.styles['Normal']
            set_para_spacing_para(p, before_twips=0, after_twips=0, line_twips=360, first_indent_twips=0)
...
```python
input_file = INPUT  # 或 os.environ.get('PAPER_FORMAT_INPUT')
output_name = '论文标题.docx'
